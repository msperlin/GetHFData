...
#' @param type.matching Type of matching for asset names in data ('exact' or 'partial')
#' @param type.market The type of market to download data from ('equity', 'equity-odds','options', 'BMF' ).
#' @param type.data The type of financial data to download and aggregate ('trades' or 'orders').
...
#' @param first.time The first intraday period to import the data. All trades/orders before this time of day are ignored. As character, e.g. '10:00:00'.
#' @param last.time The last intraday period to import the data. All trades/orders after this time of day are ignored. As character, e.g. '18:00:00'.
...
#'        The choice 'raw' outputs the raw, tick by tick/order by order, data from the zip files.
...
ghfd_get_HF_data <- function(my.assets = NULL,
  ...
  if (last.date<first.date){
    ...
    stop(paste('Found that last.date > first.date. Did you mix the two up?',
               'Please check it.'))
  }
  ...
  # check type.data
  possible.names <- c('exact','partial')
  idx <- type.matching %in% possible.names

  if (!any(idx)){
    stop(paste(c('Input type.matching not valid. It should be one of the following: ', possible.names), collapse = ', '))
  }
